...
| 12 | `12-postgres-exporter.yaml` | `Secret/postgres-exporter-secret`, `Deployment/postgres-exporter` (**`replicas: 0`**), `Service/postgres-exporter` (:9187) | PostgreSQL metrics exporter. **Scaled to zero since 2026-08-11** — there's no PostgreSQL in this cluster (the app uses MySQL); it leaked memory and OOM-looped 57 times before being disabled. `pg_up`/`PostgreSQLDown`/the PostgreSQL dashboard are dead until it's replaced with `mysqld-exporter`. |
